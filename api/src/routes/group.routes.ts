...
groupRouter.post('/group', async (req: Request, res: Response) => {
    try {
        ...
        const group = await db.createGroup({name, devices, messages} as Group);
        if (!group) {
            return res.status(StatusCodes.INTERNAL_SERVER_ERROR).json({error: "Failed to create group"});
        }
...
groupRouter.put('/group', async (req: Request, res: Response) => {
    try {
        ...
        return res.status(StatusCodes.INTERNAL_SERVER_ERROR).json({error: "Failed to update group"});
    } catch(error) {
        return res.status(StatusCodes.INTERNAL_SERVER_ERROR).json({error: error});
    }
});
