...
class IntegrationClassifier:
    def __init__(self, max_words=10000, max_length=30, embedding_dim=200):
        ...
        self.integration_indicators = {
            ...
            'communication': {
                'actions': ['send', 'receive', 'reply', 'forward', 'compose', 'call', 'message', 'chat', 'share'],
                'objects': ['email', 'message', 'call', 'voice', 'video', 'text', 'document', 'file', 'contact'],
                'platforms': ['slack', 'teams', 'zoom', 'discord', 'whatsapp', 'telegram', 'gmail'],
                'modifiers': ['urgent', 'private', 'group', 'direct', 'team', 'channel']
            }
        }
        
        self.command_patterns = {
            'actions': ['help', 'exit', 'quit', 'clear', 'show', 'list', 'version', 'about', 'info'],
        ...
        for integration_type, word_categories in self.integration_indicators.items():
            for _ in range(samples_per_category):
                if random.random() < 0.75:
                    ...
                    patterns = [
                        f"{action} {obj} on {platform}",
                        f"using {platform} {action} {obj}",
                        f"{platform} {action} {obj}",
                    ]
