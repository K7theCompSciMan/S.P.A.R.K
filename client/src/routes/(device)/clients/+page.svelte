<script lang="ts">
    ...
    let selectedDevice: Device = { id: '', name: '', messages: [], deviceCommands: [], type: 'client' };
    ...
    onMount(async () => {
        ...
        const results = await Promise.all(
            deviceIds.map(id => getDeviceApi(id, 'client', accessToken))
        );
        clients = results.filter(d => d !== null);
    });

    async function handleUpdate(device: Device) {
        const res = await updateDeviceApi(device, accessToken, thisDevice.id);
        if (res.ok) {
            clients = clients.map(c => c.id === device.id ? { ...c, ...device } : c);
        }
    }
    ...
    <div class="flex flex-col items-center h-full pt-8 px-8">
        <header class="text-center mb-8">
            <h1 class="text-3xl font-light text-slate-100">
                Client Devices <span class="text-dark-accent font-medium">@{group.name}</span>
            </h1>
            ...
            <div class="w-full max-w-4xl grid grid-cols-1 md:grid-cols-2 gap-4 overflow-y-auto no-scrollbar pb-12">
                ...
                {#each clients as client}
                    ...
                    <div 
                        ...
                        on:click|self={() => openSettings(client)}
                    >
                        <div class="flex justify-between items-start mb-4">
                            <input
                                ...
                                on:focusout={() => handleUpdate(client)}
                                on:keydown={(e) => e.key === 'Enter' && handleUpdate(client)}
                            />
                            <button 
                        ...
                        <div class="flex gap-4 mt-auto">
                            ...
                            <div class="flex items-center gap-1.5 text-xs text-dark-secondary">
                                ...
                                Commands: {client.deviceCommands?.length || 0}
                            </div>
                        </div>
                    </div>
                {/each}
